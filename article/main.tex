...
Eventhough (Ethereum) users and transactions can be deanonymized already on the network layer \cite{ethmacrofag2016}, we consider network anonymity an orthogonal problem to that of anonymity on the transaction ledger.    

\textbf{Our contributions.} In this paper, we present an efficient mixing protocol for Turing-complete blockchains. This protocol can be used to shuffle cyphertexts, ballots or public keys. The protocol have many usecases: shuffling ElGamal-cyphertexts, decentralized mixnets, e-voting.
...
We are going to conduct external code audits, deploy MixEth on the Ethereum main net and evaluate its performance. An important and crucial research question is whether it is possible to design and implement an efficient and trustless Ethereum-based coin mixer with strong security guarantees which do not rely on any workaround or upcoming EIP implementation and is ready to be deployed on present-day Ethereum. 

\section{Acknowledgements}
Chris Buckland is supported by an Ethereum Foundation
scaling grant and an Ethereum Community Fund grant.
We would like to thank Liam Horne, Patrick McCorry, barryWhiteHat, Dmitry Khovratovich and Sina Mahmoodi for the insightful comments and discussions. 
